...
const findNums2 = function (findArr, goal) {
  const resultArr = findArr.map((el, index, arr) => {
    if (index + 1 === arr.length) return;

    for (let i = 1; i < arr.length; i++) {
      if (el + arr[i + index] === goal && arr[i + index]) console.log(el);
      console.log(arr[i + index]);
      return [el, arr[i + index]];
    }
  });
...
const twoSum = function (nums, target) {
  ...
  const twoNums = findNums2(nums, target);
  console.log(twoNums);

...
function Board() {
  ...
  useEffect(() => {
    if (board) {
      ...
      const positionsForWin = possibleWin.map((set) =>
        set.map((index) => boardTypes[index])
      );
      const isWin = positionsForWin.find((positionForWin) => {
        const set = Array.from(new Set(positionForWin));
        return !set.includes(null) && set.length === 1;
      });

      const isWin = positionsForWin.find((position) =>
        position.every((v) => v !== null && v === position[0])
      );

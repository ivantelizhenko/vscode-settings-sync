...
function LetterCapitalize(phrase: string) {
  const phraseArray = phrase.split(" ");
  const phraseCapitalize = phraseArray
    .map((word) => {
      const wordArr = word.split("");
      const wordFirstLetter = wordArr[0].toUpperCase();
      const wordRest = wordArr.splice(1).join("");
      return wordFirstLetter + wordRest;
    })
    .join(" ");

  return phraseCapitalize;
}

...
function Application() {
  ...
  function handleAddContact(contact) {
    setInformationValue((prev) => [...prev, contact]);
  }
...
function LetterCapitalize(phrase) {
  ...
  const phraseFirstLettersCapitalize = phraseArray
    .map((word) => {
      const wordArray = word.split("");
      const wordFirstLetter = wordArray[0].toUpperCase();
      const wordRestLetters = wordArray.splice(1).join("").toLowerCase();
      return wordFirstLetter + wordRestLetters;
    })
    .join(" ");

  return phraseFirstLettersCapitalize;
}

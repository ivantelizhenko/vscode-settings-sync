...
function InputBox({
  ...
  const { correctAnswers, type } = useAppSelector(store => store.createTask);
  const [isChecked, setIsChecked] = useState(false);
  const [curName, setCurName] = useState('');

  useEffect(() => {
    console.log(correctAnswers[curName]);
    setIsChecked(correctAnswers[curName]);
  }, [correctAnswers, setIsChecked, curName, type]);

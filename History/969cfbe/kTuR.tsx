...
type StateType = {
  ...
  isLoading: boolean;
  allRepos: string[];
  selectedIssue: { id: string; status: AreaTypeEnum };
};
...
const initialState: StateType = {
  ...
  isLoading: false,
  selectedIssue: { id: '', status: AreaTypeEnum.ToDo },
};
...
export const getIssues = createAsyncThunk(
  ...
  async function ({ owner, repoName }: { owner: string; repoName: string }) {
    ...
    const issues = data.map((issue: any) => ({
      ...
      status: AreaTypeEnum[issue.state],
    }));
      ...
      .addCase(
        ...
        (
          ...
          const toDoArr = action.payload.issues.filter(
            issue => +AreaTypeEnum[issue.status] === AreaTypeEnum.ToDo
          );

          const inProgressArr = action.payload.issues.filter(
            issue => +AreaTypeEnum[issue.status] === AreaTypeEnum.InProgress
          );
          const doneArr = action.payload.issues.filter(
            issue => +AreaTypeEnum[issue.status] === AreaTypeEnum.Done
          );

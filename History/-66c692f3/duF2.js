...
const toggleClass = function () {
  window1Main.classList.toggle('hidden--main');
  window2Main.classList.togle('hidden--main');
  window1Aside.classList.togle('hidden--aside');
};
...
btns.forEach((btn, i, arr) => {
  btn.addEventListener('click', function (e) {
    ...
    if (!+el.dataset.page) {
      ...
      window1Main.classList.add('hidden--main');
      window2Main.classList.remove('hidden--main');
      window1Aside.classList.add('hidden--aside');
    }
  });
});

...
import { JSONAction } from './utils/helpers';
...
const createRecipeObject: (data: RecipeFromAPI) => Recipe = function (data) {
...
export const loadRecipe = async function (id: string): Promise<void> {
  try {
    const data = await JSONAction(`${API_URL}${id}`, 'GET');
    console.log(data);
...
export const loadSearchResults = async function (query: string) {
  try {
    ...
    state.search.page = 1;
    const data = await JSONAction(`${API_URL}?search=${query}`, 'GET');
...
export const uploadRecipe = async function (newRecipe: AddRecipe) {
  try {
    ...
    const data = await JSONAction(`${API_URL}?key=${KEY}`, 'POST', recipe);
    console.log(data);
  } catch (err) {
    throw err;
  }

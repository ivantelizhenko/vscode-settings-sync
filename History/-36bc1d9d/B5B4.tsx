...
function ChessEnviroment() {
    ...
    if (!onlyOneTimeGetNewGameData.current) {
      if (newGame && newGame.length === 1) {
        ...
        if (!side) {
          ...
          dispatch(setSide(sideWhite || (sideBlack as SideColor)));
        }
  ...
  // Якщо два гравці є, то запускати таймер(при перезавантажені сторінки):
  useEffect(() => {
    if (needPlayer === false) {
      dispatch(startTimer());
    }
  ...
  useInterval(
    ...
    isGameOver.is ? null : isStartTimer ? 1000 : null
  );

...
export function useToggleFilter(name: string, value: string) {
  ...
  toggleFilterState(name, nameValue.value);
  ...
  if (valueSetArray.length === 0) {
    setValueSetArray([value]);
  } else if (valueSetArray.length > 0 && comparisonIndex === -1) {
    setValueSetArray(prev => [...prev, value]);
  } else if (valueSetArray.length > 0 && comparisonIndex > -1) {
    setValueSetArray(prev => prev.filter(el => el !== value));
  }

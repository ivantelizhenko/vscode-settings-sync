...
for (let i = 0; i < newArrFlights.length; i++) {
  ...
  newArrFlights[i][3] = `(${newArrFlights[i][3]})`;
  newArrFlights[i] = newArrFlights[i].join(' ');
  newArrFlights[i] = newArrFlights[i].padStart(newArrFlights[0].length, ' ');
}

for (const word of flights.split('+')) {
  const word1 = word.split(';');
  word1[0] = word1[0].replaceAll('_', ' ');
  word1[0] = word1[0].includes('Delayed') ? '🔴' + `${word1[0]}` : word1[0];
  word1[1] = `from ${word1[1].toUpperCase().slice(0, 3)}`;
  word1[2] = `to ${word1[2].toUpperCase().slice(0, 3)}`;
  word1[3] = `(${word1[3]})`;
  word1 = word1.join(' ');
  word1 = word1.padStart(newArrFlights[0].length, ' ');
  console.log(word1.split('\n'));
}
console.log(newArrFlights.join('\n'));
// console.log(newArrFlights);
// 🔴 Delayed Departure from FAO to TXL (11h25)
//              Arrival from BRU to FAO (11h45)
//   🔴 Delayed Arrival from HEL to FAO (12h05)
//            Departure from FAO to LIS (12h30)

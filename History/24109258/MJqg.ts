...
export function getGameOverReason(chess: Chess): Reason | null {
  if (!chess.isGameOver()) return null;

  const checks: [() => boolean, Reason][] = [
    [() => chess.isCheckmate(), 'checkmate'],
    [() => chess.isStalemate(), 'stalemate'],
    [() => chess.isDrawByFiftyMoves(), 'draw by fifty‑move rule'],
  ...
  return found ? found[1] : 'unknown termination reason';
}

...
function ChessBoard() {
  ...
  const { selectedTile, board, possibleMovesForPiece } = useAppSelector(
    state => state.board
  );
  ...
  // 3) Завершення гри
  useEffect(() => {
    if (board && (selectedTile || possibleMovesForPiece.length > 0)) {
  ...
  }, [board, dispatch, possibleMovesForPiece.length, selectedTile]);

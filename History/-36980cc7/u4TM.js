...
const cartSlice = createSlice({
  ...
  reducers: {
    ...
    addIngredient: {
      prepare(pizzaId, ingredient) {
        const transformIng = ingredient.toLowercase().trim();
        return { payload: { pizzaId, ingredient } };
      },
      reducer(state, action) {
        // payload = {pizzaId, ingredient}
        ...
        if (item.ingredients.includes(action.payload.ingredient)) return;
        item.ingredients.push(action.payload.ingredient);
        item.AddIngredient.push(action.payload.ingredient);
      },
    },

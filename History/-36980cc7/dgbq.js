  ...
  reducers: {
    ...
    addIngredient: {
      ...
      reducer(state, action) {
        ...
        const item = {
          ...state.cart.find(el => el.pizzaId === action.payload.pizzaId),
        };

        if (item.ingredients.includes(transformIngredient)) return;
        item.ingredients.push(transformIngredient);
        item.addIngredient.push(transformIngredient);
      },
    },
